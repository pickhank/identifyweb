 ...
 * 调用方式
 *		object validator = _woo.validator(string formname[, object options])
 * 参数
 *		formname	要添加验证的表单名称
 ...
 *					属性名称为规则名称，cmd 为验证处理函数，arg 为函数参数列表(注意函数参数的顺序)，可用值见 _woo.validator 中的 attributes
 *				message
 *
 ...
 *			参数
 *				element		表单项目名称
 *				property	设置的属性名称，可设置的属性名见 _woo.validator 中的 attributes
 *				value		要设置的属性值
 ***********************/
...
_woo.validator = function(){
		...
		'comp'	: [
			{//初始化提示
				cmd : function(vid, rev){
					...
					if(!comp)return;
					if(!comp.value){
						return this._woocheck.init || (rev ? '\u8bf7\u5148\u8f93\u5165' + rev + ' \u9879' : '\u8bf7\u5148\u8f93\u5165\u8981\u6bd4\u8f83\u7684\u9879');
					}else{
						return this._woocheck.comp || '\u8bf7\u8f93\u5165\u5bf9\u6bd4\u5185\u5bb9';
					}
				},
				arg : 'vid,rev',
				use	: true
			...
			{//验证提示
				cmd : function(vid, must, rev){
					...
					if(comp.value && comp.value != this.value)
						return this._woocheck.warn || '\u4e24\u6b21\u8f93\u5165\u4e0d\u4e00\u81f4\uff01';
				},
				arg : 'vid,must,rev',
				use	: true
			}
	...
	function message(mode, className, message, use, options){
		var msgbox = this._woocheck.message, txt = (use ? message : (this._woocheck[mode] || options.message[mode] || message)) || '';
		if(!options.configs.notips){
			msgbox.style.display = txt || (this._woocheck.must && className == 'pass') ? '' : 'none';
	...
	function check(item, options){
		...
		item.onblur = function(e, callback){
			if($(item).is(':disabled')) {
				mode = clss = info = 'pass';
				info = message.call(this, mode, clss, info, true, options);
				return info;
			}
			//下一句解决IE的执行先后造成的BUG
			if(item.form._woo_noblur || (item._valid.ajax && item._valid.timer && item._valid.callback))return;
			clearTimeout(item._valid.timer);
			onblur && onblur.call(this, e);
			var	ajax, args, flag, info, mode, clss, rule = options.rules[this._woocheck.rule];
			if(rule.length)rule = rule[1] || rule[0] || rule;
			if(rule.cmd){
				args = rule.arg.split(/\s*,\s*/g);
				for(var key = 0; args[key]; key++)args[key] = this._woocheck[args[key]];
			}else{
				if(rule instanceof RegExp || rule instanceof Function){
					args = [this._woocheck.vid, this._woocheck.must, rule, this._woocheck.min, this._woocheck.max];
...
//					info = info ? (item._woocheck.rev || '') + info : '';		// Ajax不要加那个说明
					callback && callback.call(item, !info);
				}else{
					item._valid._ajax = true;
					item._valid._value = item.value;
					ajax = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject('MSXML2.XMLHTTP');
					ajax.onreadystatechange = function(){
						//IE6 bug this == window，this 指针绑定错误
						if(ajax.readyState == 4){
							info = trim(ajax.responseText);
							if(info == 'succeed')info = '';
							item._valid.ajax[item.value] = info;
							if(item._valid._ajax){
								mode = 'ajax';
								clss = ajax.status != 200 || info ? 'warn' : 'pass';
//								info = info ? (item._woocheck.rev || '') + info : '';		// Ajax不要加那个说明
								message.call(item, mode, clss, info, undefined, options);
							}
							callback && callback.call(item, !info);
						}
						item._valid.callback = 0;
					};
					item._valid.callback = callback;
					item._valid.timer = setTimeout(function(){
						item._valid.timer = 0;
						ajax.open('GET', options.ajax[item.name].url.replace(/%1/g, encodeURIComponent(item.value)), true);
						ajax.send();
					}, 127);
					mode = clss = 'wait';
					info = '\u67e5\u8be2\u4e2d...';
				...
				info = info ? (this._woocheck.rev || '') + info : '';
			}
...
//			item.form._woo_submit = false;
			!init && onfocus && onfocus.call(item, e);
			var info, rule = options.rules[item._woocheck.rule];
			if(rule.length && rule[0]){
				rule = rule[0];
				if(rule.cmd){
					args = rule.arg.split(/\s*,\s*/g);
					for(var key = 0; args[key]; key++)args[key] = item._woocheck[args[key]];
				}else{
					if(rule instanceof RegExp || rule instanceof Function){
						args = [item._woocheck.vid, item._woocheck.must, rule, item._woocheck.min, item._woocheck.max];
						rule = {cmd : checkbyregx};
					}else{
						return;
					}
		...
		show.call(item, {}, true);
		if(item._woocheck.rule != 'void')item.onfocus = show;
	}
	function attribute(item, key, val, options){
		var item = this.elements[item];
		if(item && item._woocheck)item._woocheck[key] = val;
	...
	function loadinit(){
		...
		while(item = elem[i++]){
			if(!item._woocheck && (rule = item.getAttribute('rule'))){
				item._valid = {};
				item._woocheck = {
					'item' : item,
					'rule' : rule.toLowerCase()
				};
				each(['offset'].concat(attributes), function(){item._woocheck[this] = item.getAttribute(this)});
				item._woocheck.must = !empty(item._woocheck.must);
				if(!options.configs.notips){
					if(item._woocheck.offset && (parent = document.getElementById(item._woocheck.offset))){
						parent.appendChild(item._woocheck.message = _('DIV'));
					}else{
						parent = item.parentNode;
						offset = item;
						if(item._woocheck.offset = parseInt(item._woocheck.offset)){
							key = item._woocheck.offset > 0 ? 'nextSibling' : 'previousSibling';
							while(item._woocheck.offset && offset[key]){
								offset = offset[key];
								offset.nodeType == 1 && (item._woocheck.offset > 0 ? item._woocheck.offset-- : item._woocheck.offset++);
							}
						}
						parent.replaceChild(item._woocheck.message = _('DIV'), offset);
						parent.insertBefore(offset, item._woocheck.message);
					}
				}
				if(item._woocheck.rule != 'void'){
					check(item, options);
				}
	...
	return function(formid, options, forceResetFlag){
		if(!exists[formid] || forceResetFlag){
			...
			listen(formid, 'ready', function(){
				...
				this.onsubmit = function(e){
					...
					if(form._woo_submit && (typeof(_woo_resubmit) == 'undefined')){
						sleep = interval - Math.floor(((new Date).getTime() - form._woo_submit) / 1000);
						if(sleep > 0){
							form._woo_noblur = true;
							alert('\u6b63\u5728\u63d0\u4ea4\u8868\u5355\uff0c\u8bf7\u4e0d\u8981\u91cd\u590d\u63d0\u4ea4\uff01\n(\u5982\u679c ' + sleep + ' \u79d2\u5185\u8fd8\u672a\u63d0\u4ea4\u6210\u529f\uff0c\u53ef\u5c1d\u8bd5\u518d\u6b21\u63d0\u4ea4)');
							return form._woo_noblur = false;
						}
					}
					form._woo_submit = timestamp;
					while(item = elem[i++]){
						...
						if(options.ajax[item.name]){// && (!item._valid.ajax || !(item.value in item._valid.ajax) || item._valid._value != item.value || item._valid.ajax[item.value])
							...
							if(info = item.onblur.call(item, e, function(flag){
								...
								setTimeout(function(){
									if(!--ajax && !message){
										//最后一个Ajax通知
										if(doSubmit && form._woo_submit == timestamp){
											...
											form._woo_noblur = true;
											form._woo_submit = false;
											form._woo_noblur = false;
											first.focus();
...
function checkdroplist(id, notnull, maxNum, minNum) {
	var vals = $($id(id)).val();
	if(vals == null || vals.length==0)return empty(notnull) ? '' : '\u4e0d\u80fd\u4e3a\u7a7a';
	if(maxNum < vals.length && !empty(maxNum)) {
		return '\u6700\u5927\u6570\u91CF\u4E0D\u80FD\u8D85\u8FC7 ' + maxNum + '\u4E2A';
	}
	if(minNum > vals.length && !empty(minNum)) {
		return '\u81F3\u5C11\u9009\u62E9' + minNum + '\u4E2A';
	}
	return '';
}
