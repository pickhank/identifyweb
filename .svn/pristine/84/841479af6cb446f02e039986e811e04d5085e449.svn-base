...
<form class="search-form" action="<c:url value="/admin/downMerchantBalance/index" />" method="post" style="min-height: 30px;">
	...
	<div class="ms-col-fix-100 clear">
		<label for="balanceNo">记账流水：</label>
		<input type="text" class="input-box search-input-box" placeholder="账单流水号" name="balanceNo" value="${search.balanceNo}"></input>
	</div>
	<div class="ms-col-fix-100 clear">
		<label for="referTransNo">业务流水：</label>
		<input type="text" class="input-box search-input-box" placeholder="业务流水号" name="referTransNo" value="${search.referTransNo}"></input>
	</div>
	<div class="ms-col-fix-100 clear">
	...
	<div class="ms-col-fix-150 clear">
		<label for="startDate" class="custom-d-cate">记账日期：</label>
...
<form action="<c:url value="/admin/downMerchantBalance/export" />" target="_blank" method="post" class="export-form">
	<input type="submit" value="导出" class="btn btn-orange export-btn" ></input>
</form>
<table class="list-wapper">
	<tr class="list-header">
		<td align='center'>机构号</td>
		<td align='center'>机构名称</td>
		
		<td align='center'>记账流水</td>
		<td align='center'>资金账户</td>
		
		<td align='center'>业务类别</td>
		<td align='center'>业务流水</td>
		
		<td align='center'>原有余额（元）</td>
		
		<td align='center'>变动额度（元）</td>
		<td align='center'>现在余额（元）</td>
	...
	<c:forEach var="item" items="${res.items}" varStatus="status">
		<tr class="list-item">
			...
			<td><c:out value="${item.balanceNo}" /></td>
			...
			<td align="right">
				<label><fmt:formatNumber value="${item.balAmt }" pattern="#,##0.00#"/>&nbsp;</label>
			</td>
			<td>
