...
if(typeof M_ROOT == 'undefined'){
	...
	M_ROOT = M_ROOT.replace(/resources\/js\/admina\/floatwin\.js$/i, ''); 
}
var _ua = function(){
...
function showloading(display,waiting) {
	...
	l.innerHTML='<img src="' + M_ROOT + 'resources/images/admina/common/loading.gif" height="20" align="middle"> '+waiting;
...
function clearDelay(win){
	_WOO_.get('window').clearTimeout(window['floatwinTimer_' + win]);
}
...
var get_obj;
try { get_obj = _WOO_.get('window'); } catch(err) { get_obj = window; }
if(get_obj !== window){
	var k, fwin = _WOO_.get('floatwin');
	...
	try { me = _WOO_.get('floatwin') } catch(err) { me = this; }
	if(!me.list)me.list = {};
	if(command.match(/_$/)){
...
_WOO_.set('floatwin', floatwin);
_WOO_.set('showloading', showloading);
if(_WOO_.get('window') !== window)showloading = _WOO_.get('showloading');
...
function Ajax(recvType,waitId) {
	...
	aj.showLoading=function() {
		if(aj.waitId && (aj.XMLHttpRequest.readyState !=4 || aj.XMLHttpRequest.status !=200)) {
			...
			aj.waitId.innerHTML='<span><img src="' + IXDIR+'resources/images/admina/common/loading.gif"> ' + aj.loading + '</span>';
		}
	}
	...
	// 用适配器模式重构AJAX请求（用PHP做代理请求）
	aj.myGet=function(targetUrl,resultHandle,flag) {
		if(flag === undefined) return js_callback(targetUrl, resultHandle);
		aj.get(CMS_URL + 'tools/ajax.php?action=AjaxProxy&url=' + encodeURIComponent(targetUrl),resultHandle,flag);
	}
	// myPost跨域请求方式 // zjh添加
	aj.myPost=function(targetUrl,sendString,resultHandle){
		aj.post(targetUrl,sendString,resultHandle);
	}
