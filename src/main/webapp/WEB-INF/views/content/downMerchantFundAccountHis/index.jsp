...
<form class="search-form" action="<c:url value="/content/downMerchantFundAccountHis/index" />" method="post">
	...
	<a href="<c:url value="/content/downMerchantFundAccountHis/add" />" class="btn btn-orange pull-right operate-detail" >新建</a>
</form>
<table class="list-wapper">
	...
	<c:forEach var="item" items="${res.items}" varStatus="itemStatus" >
		<tr class="list-item">
			...
			<td>
				<a href="<c:url value="/content/downMerchantFundAccountHis/add?id=${item.id}" />" class="choose-link operate-detail">编辑</a>
				<!-- a href="<c:url value="/content/downMerchantFundAccountHis/delete/${item.id}" />" class="operate-delete">删除</a-->
			</td>
		</tr>
	</c:forEach>
</table>
