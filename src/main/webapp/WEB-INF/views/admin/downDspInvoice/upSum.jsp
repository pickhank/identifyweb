...
<form class="search-form" action="<c:url value="/admin/downDspInvoice/upSum" />" method="post">
	...
	<utils:dcombo name="channel" cssName="input-box search-input-box" value="${search.channel}"></utils:dcombo>
...
<form action="<c:url value="/admin/downDspInvoice/exportUpSum" />" target="_blank" method="post" class="export-form">
	...
	<c:forEach var="item" items="${items}" varStatus="status">
		<tr class="list-item">
			<td style="text-align:left;">
				<c:out value="${item.channelAccName}" />(<utils:combo mode="lbl" value="${item.channel}" />)
			</td>
			<td>
				<utils:enum mode='lbl' key="EnumDspChannel" value="${item.dspMode }" />
			</td>
			<td class="warning">
			...
			<c:set var="mchFeeAmt" value="${mchFeeAmt + item.mchFeeAmt }" />
			<c:set var="agentProfitAmt" value="${agentProfitAmt + item.agentProfitAmt }" />
			<c:set var="profitAmt" value="${profitAmt + item.profitAmt }" />
		</tr>
	</c:forEach>
