...
<form class="search-form" action="<c:url value="/admin/downMerchantDeposit/upSum" />" method="post">
...
<form action="<c:url value="/admin/downMerchantDeposit/exportUpSum" />" target="_blank" method="post" class="export-form">
...
<table class="list-wapper">
	<tr class="list-header">
		...
		<td>成功率</td>
		<td align='right'>总金额（元）</td>
		<td align='right'>总笔数</td>
		
		<td align='right'>待查金额（元）</td>
		<td align='right'>待查笔数</td>
		
		<td align='right'>成功金额（元）</td>
		<td align='right'>成功笔数</td>
	...
	<c:forEach var="item" items="${items}" varStatus="status">
		<tr class="list-item">
			...
			<td>
				<c:choose>
					<c:when test="${item.mode eq '0' }">T1账户</c:when>
					<c:when test="${item.mode eq '1' }">T0账户</c:when>
